            ...
            >
                ...
                <template v-else>
                    <template v-for="col in columns">
                        ...
                        <el-table-column v-if="!col.slots" :key="col.prop" v-bind="col"></el-table-column>
                ...
                <el-table-column label="操作" v-if="hasOperation" v-bind="operationAttrs">
                    <template slot-scope="scope">
                        <text-button v-if="isTree && hasNew" @click="onDefaultNew(scope.row)">{{ newText }}
                        </text-button>
...
<script>
    ...
    export default {
        ...
        methods: {
             ...
             */
            getList(saveQuery) {
                // this.data = [{name: "pipi", age: 18, count: 1}, {name: "pipi", age: 18, count: 1}, {
                //     name: "pipi",
                //     age: 18,
                //     count: 1
                // }];
                ...
                //如果requests有axios和getData，使用它的方法获取请求
                const {axios, getData} = this.requests;
                if (getData && axios) {
                    try {
                        let {url: urlInfo = url, method = "get", errorHandler, successHandler} = getData;
                        //调用配置进来的函数，方便调用者进行做数据处理
                        axios[method](urlInfo + queryStr).then(res => {
                            successHandler ? successHandler(res, _getThen) : _getThen(res);
                        }).catch(err => {
                            errorHandler ? errorHandler(err, _getCatch) : _getCatch(err);
                        });
                    } catch (e) {
                        _getCatch(e);
                    }
